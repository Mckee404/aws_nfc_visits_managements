...
func getVisits(c *gin.Context) {
	...
	keyCond := expression.KeyAnd(
		expression.Key("PK").Equal(expression.Value(fmt.Sprintf("BOOTHS#%s", boothID))),
...
func getVisitors(c *gin.Context) {
	...
	// --- Build DynamoDB Query ---
	// Assumes a GSI named "EntityTypeIndex" with PK=EntityType
	keyCond := expression.Key("EntityType").Equal(expression.Value("VISITOR"))
	var filterConditions []expression.ConditionBuilder

	if name != "" {
		filterConditions = append(filterConditions, expression.Name("name").Contains(name))
	}
	...
	if len(filterConditions) > 0 {
		...
		for i := 1; i < len(filterConditions); i++ {
			filter = filter.And(filterConditions[i])
		}
